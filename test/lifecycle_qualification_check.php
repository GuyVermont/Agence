...
$tracking = new SofAvoirTracking($db);
$tracking->entity = (int) $conf->entity;
$tracking->ref = 'TEST-LIFE-AVO-'.$token;
$tracking->fk_facture_avoir = (int) $creditId;
$tracking->fk_facture_origin = (int) $invoiceId;
$tracking->fk_soc = (int) $thirdParty->rowid;
$tracking->fk_agence = (int) $agencyId;
$tracking->fk_session = (int) $sessionId;
$tracking->initial_amount = 1000;
$tracking->used_amount = 0;
$tracking->remaining_amount = 1000;
$tracking->validation_status = 0;
$tracking->use_status = 0;
$tracking->status = 0;
$trackingId = $tracking->create($user, 1);
$trackingValidation = $engine->validateCreditTracking($user, $trackingId);
